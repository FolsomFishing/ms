jQuery(document).ready(function(){
        ...
        wisdm_obj[object_name.sourceimageurlforzoom] = object_name.sourceimagesinglesrc;
		...
		type: "POST",
		traditional: true,
		url: object_name.admin_ajax_url,
		async: false,
		data: data,
		...
		function load_rest_of_script(){
			...
			jQuery('.first-featured-image img').attr({
				src: object_name.sourceimageurl,
				width: object_name.thumbnail_image_width,
				height: object_name.thumbnail_image_height,
				style: ""
				});
			...
			jQuery("#slider_main_div").css({'overflow':'hidden', 'height':object_name.thumbnail_image_height, 'width': jQuery(".images").width()});
			jQuery(".bxslider li img").width(object_name.thumbnail_image_width).height(object_name.thumbnail_image_height);
			...
			jQuery(".bxslider li .wisdm_zoom").click(function()
			{
				...
				first_image_selector.find('img:first').attr("src", wisdm_obj[get_original_link_of_image]).width(jQuery(".images").width()).load(function(){
				var zoom_level_to_be_set_new = parseInt(object_name.zoomleveloption) || 2;
